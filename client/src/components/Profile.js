...
function Profile({ user }) {
  ...
  const filteredInterventions = interventions?.filter(
    (intervention) => intervention?.user?.id === user?.id
  );
    ...
    <div>
      ...
      >
        <Card.Body>
          ...
          <div style={{ display: "flex", flexDirection: "row" }}>
            <Button variant="danger" onClick={() => setReportToggle(true)}>
              View Red Flags
            </Button>
      ...
      {reportToggle ? (
        ...
        <Table striped bordered hover style={{ marginTop: "20px" }}>
          ...
          <tbody>{userInterventionList}</tbody>
        </Table>
      )}
    </div>
  );
